...
4. GitLab CI/CD variables: `AWS_ROLE_TO_ASSUME` and `AWS_REGION`
...
1. A Google Cloud project with Vertex AI API enabled, Workload Identity Federation configured to trust GitLab OIDC
2. A dedicated service account with only the required Vertex AI roles
3. GitLab CI/CD variables: `GCP_WORKLOAD_IDENTITY_PROVIDER`, `GCP_SERVICE_ACCOUNT`
...
* **API costs**: Each Claude interaction consumes tokens based on prompt and response size. See [Anthropic pricing](https://platform.claude.com/docs/en/about-claude/pricing) for details
