...
# Batch size for ASR inference.
ASR_BATCH_CPU = 4
ASR_BATCH_CUDA = 16
# Duration (seconds) of each audio chunk we feed to qwen_asr.
...
def ensure_asr_model(
    ...
    Returns:
        Tuple of (asr_model, model_name, batch_size).
    """
    if current_model is not None and current_name == model_name:
    ...
    try:
        ...
        return None, model_name, ASR_BATCH_CPU
    ...
    return asr_model, model_name, batch_size
...
def run_faster_whisper(
    ...
    model_name: str,
    cached_model=None,
    cached_model_size: str | None = None,
) -> tuple[list[dict], Any, str]:
    """Run faster-whisper ASR on a video file.

    Returns:
        Tuple of (transcript, model_instance, model_size) so the caller
        can cache the model for repeated calls.
    """
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not extract_audio(video_path, wav_path):
            return [], cached_model, model_size
...
def run_asr(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        chunks = split_audio_chunks(
            wav_path, ASR_CHUNK_SEC, tmp, overlap_sec=overlap,
        )
        ...
        logger.info(
            ...
            len(chunks), ASR_CHUNK_SEC, overlap,
        )
...
def get_transcript(
    ...
    faster_whisper_model_size: str | None = None,
) -> tuple[list[dict], Any, str | None]:
    """Return ASR transcript as a list of ``{start_time, end_time, text}`` dicts.

    Returns:
        Tuple of (transcript, faster_whisper_model, faster_whisper_model_size)
        so the caller can cache the faster-whisper model.
    """
    if transcript_path is not None:
        return load_transcript_file(transcript_path), faster_whisper_model, faster_whisper_model_size

    if is_faster_whisper_model(asr_model_name):
        result, fw_model, fw_size = run_faster_whisper(
            video_path, asr_model_name, faster_whisper_model, faster_whisper_model_size
        )
        return result, fw_model, fw_size

    result = run_asr(video_path, asr_model_name, asr_model, asr_batch_size)
    return result, faster_whisper_model, faster_whisper_model_size
