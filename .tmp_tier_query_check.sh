...
run() {
  local label="$1" tier="$2" query="$3"
  local result tier_used confidence top status
  result=$(timeout 40 "$KUAVI" query --video "$TEST_VIDEO" --question "$query" --max-tier "$tier" --output-format jsonl 2>/dev/null || true)

  tier_used=$(printf '%s\n' "$result" | grep '"type": "cost"' | head -n1 | grep -o '"tier_used": [^,}]*' | grep -o '[0-9.]*')
  confidence=$(printf '%s\n' "$result" | grep '"type": "result"' | head -n1 | grep -o '"confidence": [^,}]*' | grep -o '[0-9.]*')

  top=$(printf '%s\n' "$result" | "$PY" -c 'import sys,json
lines=sys.stdin.read().splitlines(); r=None
for ln in lines:
    try:
        d=json.loads(ln)
    except Exception:
...
if isinstance(f,dict):
    v=f.get("start",f.get("start_time",f.get("timestamp",f.get("time","-"))))
...
for item in "find the scene where someone picks up an object|2" "what is happening at minute 3|1"; do
  q="${item%|*}"; t="${item#*|}"
  out=$(timeout 40 "$KUAVI" query --video "$TEST_VIDEO" --question "$q" --max-tier "$t" --output-format jsonl 2>/dev/null || true)
  conf=$(printf '%s\n' "$out" | grep '"type": "result"' | head -n1 | grep -o '"confidence": [^,}]*' | grep -o '[0-9.]*')
  [ -z "$conf" ] && conf="0.000"
  "$PY" - <<PY
c=float("$conf")
